"""Integration tests for MaterialClassifier with Anthropic adapter.

These tests exercise the full MaterialClassifier V4 pipeline using the
real Anthropic Claude Vision models when RUN_INTEGRATION_TESTS=1 and
...
def _load_image_bytes(name: str) -> bytes:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"Missing fixture image for MaterialClassifier Anthropic test: {path}. "
...
requires_integration = pytest.mark.skipif(
    not os.getenv("RUN_INTEGRATION_TESTS") or not os.getenv("ANTHROPIC_API_KEY"),
    reason="Integration tests disabled or ANTHROPIC_API_KEY not set",
)


@requires_integration
@pytest.mark.asyncio
async def test_material_classifier_anthropic_pet_bottle() -> None:
    """
    ...
    result = await classifier.classify(image_bytes, "trace-anthropic-integration")
