...
Tests the complete pipeline flow including all 7 agents:
1. PreValidator
...
def mock_validation_result() -> ValidationResult:
    """Create a mock validation result (waste detected)."""
    return ValidationResult(
        is_valid=True,
        reason=ValidationReason.WASTE_DETECTED,
        metadata={"detections": [{"class": "plastic", "confidence": 0.9}]},
        cost=0.001,
        fallback_used=False,
    )
...
class TestPipeline:
    ...
    async def test_pipeline_initialization(self):
        """Test pipeline initializes all 7 agents correctly."""
        pipeline = Pipeline()

        # Check all agents initialized
        assert pipeline.pre_validator is not None
    ...
    async def test_pipeline_complete_flow(
        ...
        """Test complete pipeline flow with all 7 agents."""
        pipeline = Pipeline()
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    assert len(response.meta.agents_executed) == 7
    ...
    @pytest.mark.asyncio
    async def test_pipeline_no_waste_detected(
        self,
        mock_request: ClassifyRequestForm,
    ):
        """Test pipeline when PreValidator rejects (no waste detected)."""
        pipeline = Pipeline()

        # Mock PreValidator to reject
        ...
        with patch.object(
            ...
            mock_validate.return_value = reject_result
            ...
            assert "residuo" in exc_info.value.message.lower()
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    # Verify trace_id passed to agents
    ...
    async def test_pipeline_cost_calculation(self):
        ...
        # V4 cost should be PreValidator + MaterialClassifier
        # = $0.001 + $0.010 = $0.011
        assert cost == pytest.approx(0.011, rel=0.01)
        assert cost < 0.015  # Within reasonable bounds

    @pytest.mark.asyncio
    async def test_pipeline_with_invalid_input(self):
        """Test pipeline with invalid input (no image_bytes or image_url)."""
        pipeline = Pipeline()
        ...
        invalid_request = ClassifyRequestForm(
            scan_id=uuid4(),
            station_id="TEST-STATION-01",
            image_bytes=None,  # No image bytes
            tenant_id="test-tenant",
            trace_id=uuid4(),
            idempotency_key=uuid4(),
        )
