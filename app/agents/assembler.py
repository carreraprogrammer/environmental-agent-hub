"""
Assembler Agent - Builds the final ClassifyResponse from agent outputs.

This agent is responsible for consolidating outputs from all 9 previous agents
in the pipeline into a single, validated ClassifyResponse object.
...
class Assembler:
    """
    Assembles the final ClassifyResponse from agent outputs.

    This agent consolidates outputs from all agents in the classification
    pipeline into a single, validated response object. It performs no
    business logic - only data assembly and validation via Pydantic.

    The assembler is synchronous because it performs no I/O operations.
