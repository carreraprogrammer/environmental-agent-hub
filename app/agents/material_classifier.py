...
class MaterialClassifier:
    ...
    Classifies:
    - Material base type (NO_WASTE, PLASTIC, PAPER, GLASS, METAL, ORGANIC, CARDBOARD, TETRAPAK)
    ...
    def _parse_result(
        ...
        # Validate material confidence (allow NO_WASTE even with lower confidence to let pipeline handle it)
        if material_type != Material.NO_WASTE and material_confidence < self.MIN_MATERIAL_CONFIDENCE:
            raise ValueError(
        ...
        # NO_WASTE is a terminal outcome: no partial success semantics needed
        if material_type == Material.NO_WASTE:
...
PASO 0 - DETECCIÓN:
- Si NO hay residuo en la imagen, responde SOLO:
  {
    "material": {"type": "NO_WASTE", "confidence": 0.99},
    "subtype": {"value": null, "recycling_code": null, "confidence": 0.0},
    "condition": {"value": "CLEAN", "confidence": 0.0},
    "volume": {"liters": null, "source": "ESTIMATED", "confidence": 0.0},
    "recyclability": {"value": "NON_RECYCLABLE", "confidence": 0.0},
    "reasoning": "No se detecta residuo en la imagen"
  }
  No hagas más inferencias.

Debes identificar:

1. **Material Base** (OBLIGATORIO):
   - NO_WASTE: No hay residuo u objeto relacionado con residuos
